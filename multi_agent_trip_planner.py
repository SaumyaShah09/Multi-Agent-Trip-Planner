...
def generate_full_itinerary(prefs):
    ...
    flight_task = Task(description=f"""
        Here are the best flights from {prefs['departure']} to {prefs['destination']}:
        {flights_txt}
    """, expected_output="Flight summary", agent=flight_finder)

    hotel_task = Task(description=f"""
        Here are 3 top hotels in {prefs['destination']} for a {prefs['budget']} budget:
        {hotels_txt}
    """, expected_output="Hotel summary", agent=hotel_finder)

    itinerary_task = Task(description=f"""
    You are an expert travel planner. You must create a {prefs['days']}-day itinerary for a luxury trip from {prefs['departure']} to {prefs['destination']}.
    ...
    Strict Instructions:
    - Do NOT invent any explanations or assumptions like "assuming flight is booked", or hotel name clarifications.
